...
def treat_bowtie_in(resultFile,e,dic_seq,genome,len_sgrna,gene_coords): 
...
def add_in_parallel(num_thread,list_fasta,organism_code,dic_seq,genome,len_sgrna,gene_coords):
    ...
    def worker():
        while True:
            ...
            treat_bowtie_in(resultFile,e,dic_seq,genome,len_sgrna,gene_coords)
            q.task_done()
...
def do_search(query_seq, n, genome_list, dict_org_code, not_in_genome_list,
    ...
    distDic=cf.readJsonDic(distFile)
    ...
    for i in list_order:
        genome=i[0]
        if i[1]=='notin':
            dic_seq=cf.add_notin_parallel(num_threads,list_fasta,dict_org_code[genome][0],dic_seq)
            if len(dic_seq)==0:
...
def main():
    ...
    dict_organism_code = cf.readJsonDic(REF_GEN_DIR + '/genome_ref_taxid.json')
